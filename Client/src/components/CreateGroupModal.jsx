...
const CreateGroupModal = ({ setShowGroupModal, setGroups }) => {
    ...
    const addUserField = () => {
        setFormData((prev) => ({
            ...prev,
            users: [...prev.users, { name: "", email: "", role: "member" }],
        }));
    };
    ...
    const handleCreateGroup = async () => {
        const validationErrors = CreateGroupValidate({
        ...
        const hasErrors =
            ...
            (validationErrors.groupName ||
                (validationErrors.users &&
                    validationErrors.users.some((u) => Object.keys(u).length > 0)));
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
            <div className="relative w-full max-w-md p-6 rounded-3xl shadow-2xl backdrop-blur-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/20">
                ...
                >
                    ...
                    >
                        <Plus size={16} />
                        {showUsers ? "Hide Users" : "Add Users (Optional)"}
                    </button>
                    ...
                    <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className={`px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white ${loading ? "opacity-50 cursor-not-allowed" : ""
                                }`}
                        >
                            {loading ? "Creating..." : "Create Group"}
                        </button>
